...
#### Then transcribe (same for both methods):
...
segments, info = model.transcribe('/tmp/ig_audio_16k.wav', language='en')
for seg in segments:
...
- `faster-whisper` — installed via pip (SDK, no CLI needed)
