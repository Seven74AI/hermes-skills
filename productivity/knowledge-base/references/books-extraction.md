...
- **Same-topic books can be misidentified.** When processing a batch of books on the same subject (e.g., 3 books about urine therapy), workers may load the right file but misidentify its content as a different book in the batch. **Always verify the EPUB metadata (title + author) matches the book you're supposed to process BEFORE writing the note.** If a worker reports "metadata discrepancy — file X contains book Y," re-verify by extracting the EPUB yourself — the worker may be wrong. Real case (2026-06-09): Scazzero file was correctly labeled but worker claimed it contained Bhurani's content because both discuss urine therapy.
