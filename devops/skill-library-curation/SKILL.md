...
7. **Agent skills with an unmarked usage record are invisible to `run`/`prune`.** `list_agent_created_skill_names()` gates on `_is_curator_managed_record` (`created_by == "agent"` OR `agent_created is True`). A skill authored via an older path or a script can land on disk with `created_by: null` — it then never gets marked stale or archived automatically, and `prune --dry-run --days 90` reports "nothing to prune" even though the skill is idle 90d+. Cross-check `hermes curator usage` (lists ALL on-disk skills with provenance) against `agent_created_report()`: any agent skill idle ≥90d that's absent from the prune candidate list needs `hermes curator archive <name>` by hand (`archive_skill` checks `is_curation_eligible`, not the managed-record flag, so it works).

8. **Phantom `category/name` keys in `.usage.json` false-positive the Pitfall-7 cross-check.** After a skill is moved/renamed into a category (e.g. `shop` → `dogfood/shop`), `.usage.json` can retain BOTH a `"dogfood/shop"` record (`created_by: null`, stale timestamps) and a canonical `"shop"` record (`agent_created: true`). The prefixed record is a rename leftover, not a real skill — its `idle >= 90d` look makes the cross-check falsely report "archive candidate". Before archiving anything, confirm the *bare-name* key exists and is the canonical one; if a bare-name `agent_created:true` twin exists and is active, the prefixed key is a phantom and must be ignored (or removed by exact name), NOT archived. Archiving would hit the live skill.
