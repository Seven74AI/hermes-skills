...
**Two distinct root causes produce the same ``Unknown skill(s)`` symptom:**

**(A) Dangling symlinks** — Skills in profile directories (``~/.hermes/profiles/<assignee>/skills/``) were synced from a temporary source that has since been deleted. The ``_validate_and_sync_skills()`` function (kanban_db.py:5464) copies skills to the profile at task creation time, resolving symlinks to their targets. If the target directory is later cleaned up (e.g. ``/tmp`` expiry), the synced symlinks become dangling — the skill directory exists but its contents are unreachable.

**(B) Skill name collision** — Two installs of the same skill exist: one as a top-level symlink (e.g. ``~/.hermes/skills/tdd → .../mattpocock-skills/.../tdd``) and one as a categorized copy (e.g. ``~/.hermes/skills/software-development/tdd/SKILL.md``). The skill resolver sees two matches for the bare name and refuses to choose — the error is ``Unknown skill(s): tdd`` even though the SKILL.md files exist and are valid. Same exact diagnostic visible in kanban logs: 40+ consecutive ``Unknown skill(s)`` errors every ~60s. The fix: remove the duplicate (prefer the symlinked Matt Pocock version over the local copy). Check BOTH ``~/.hermes/skills/`` AND ``~/.hermes/profiles/<assignee>/skills/`` — the collision can exist at either level. After removal, verify resolution with ``skill_view <name>`` — it should return without ``Ambiguous skill name``.
...
**Real case (2026-07-08, music-library board):** Collision (B) — ``tdd`` existed both as a symlink (``~/.hermes/skills/tdd → mattpocock-skills``) and as a duplicate copy (``~/.hermes/skills/software-development/tdd/SKILL.md``). Same duplicate existed in the ``coder`` profile. Tasks t_11ca1f80 and t_63cd171c did 40+ consecutive crashes over 40 minutes with ``Unknown skill(s): tdd``. The intra-tick fingerprint detector never fired because only one crash happened per dispatch tick. Fix: removed ``software-development/tdd`` from both locations.
