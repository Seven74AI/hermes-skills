...
**⛔ Coder tasks MUST include `kanban-project-workflow` in skills.** Every task assigned to `coder` must have `--skill kanban-project-workflow` (plus any project-specific skills). Without it, coders don't know the PR workflow rules — they'll post full PR URLs in comments, triggering the 24h `active_pr` respawn guard and deadlocking the board. Project skills (e.g. `music-library`) may list additional required skills — always load the project skill before creating coder tasks.
...
**⛔ Never assume the user wants a destructive action — always ask first.** When you identify a fix that requires a gateway restart, DB modification, profile deletion, or any other non-trivial system change, present the fix and ASK before executing. Do NOT say "I'll restart it" or "let me just do X" — the user may have active work, long-running sessions, or reasons to defer. Silent consent is NOT consent. **Real case (2026-07-13):** agent proposed gateway restart as first action without asking; user pushed back with "Do not fucking assume." The fix (skill collision removal) was correct but the assumption was wrong.
...
**WAL mode is the default — not DELETE.** Hermes kanban DBs use WAL journal mode by default (`hermes_state.py` line 152: `PRAGMA journal_mode=WAL`). DELETE mode is only a fallback for NFS/SMB/FUSE filesystems where WAL doesn't work. The music-library DB remained WAL through July 2026 as evidence. **The theory that long gateway uptime causes WAL corruption has been debunked by the user multiple times — do NOT diagnose dispatcher stalls as WAL corruption.** See `references/dispatcher-stuck-diagnosis.md` for the correct diagnostic path.

**⛔ Dispatcher stuck: "ready queue non-empty but 0 workers spawned" — diagnosis priority.** When the dispatcher logs this warning, follow the diagnostic steps in `references/dispatcher-stuck-diagnosis.md`. The most common root cause (and the FIRST thing to check) is **skill name collisions in worker profiles** — a duplicate skill in `~/.hermes/profiles/<assignee>/skills/` causes workers to crash silently on startup with `Unknown skill(s)`. The dispatcher counts these as spawn failures, hits `failure_limit: 3`, and stops spawning. The global skills dir (`~/.hermes/skills/`) and the profile's skills dir (`~/.hermes/profiles/<assignee>/skills/`) must not contain skills with the same name. Remove the non-canonical copy (usually the older/stale one in the profile). A gateway restart may still be needed after fixing the collision to clear the dispatcher's stuck state. **Real case (2026-07-13):** `grill-with-docs` existed in both `~/.hermes/skills/software-development/` and `~/.hermes/profiles/coder/skills/software-development/` — dispatcher stuck for 4+ hours spawning 0 workers despite `max_spawn=3` and 5 ready tasks.

**Full diagnostic recipe:** When the user asks "why only N workers?" or "pourquoi ça n'avance pas?", follow the 3-command path in `references/dispatcher-stuck-diagnosis.md` — covers DB corruption, skill collisions, and profile health as root causes.
